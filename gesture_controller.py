...
def process_frame(frame, detector_mp, gesture_detector, keyboard_ctrl, ws_server, last_gesture, mode):
    ...
    # Broadcast gesture event (legacy compatibility)
    ...
    # --- SPELLCASTER ARENA ENHANCED PAYLOAD ---
    hand_data_list = []
    for idx, hl in enumerate(hands):
        side = handedness_list[idx][0].category_name if idx < len(handedness_list) else "Unknown"
        # Gather normalized landmarks
        lm_list = [{"x": lm.x, "y": lm.y, "z": lm.z} for lm in hl]
        # Basic features
        palm_cx = sum([lm.x for lm in hl[0:1]+hl[5:17:4]]) / 5
        palm_cy = sum([lm.y for lm in hl[0:1]+hl[5:17:4]]) / 5
        palm_cz = sum([lm.z for lm in hl[0:1]+hl[5:17:4]]) / 5
        
        fingers = gesture_detector._fingers_up(hl)
        hand_data_list.append({
            "handedness": side,
            "landmarks": lm_list,
            "palmCenter": {"x": palm_cx, "y": palm_cy, "z": palm_cz},
            "palmRadius": 0.05,
            "fingersUp": fingers,
            "isFist": sum(fingers) == 0,
            "isPinch": False, # Basic for now
            "isOpenPalm": sum(fingers) == 5,
            "isPointing": fingers == [0, 1, 0, 0, 0] or fingers == [1, 1, 0, 0, 0],
            "velocity": {"x": 0, "y": 0}
        })
    ...
    spell_state = {
        ...
        "gestures": [gesture.value],
        "timestamp": time.time()
    }
    
    # Broadcast raw state to all clients directly (without a dedicated dataclass to save time)
    class SpellEvent:
        def to_json(self):
            return json.dumps(spell_state)
    
    ws_server.broadcast(SpellEvent())
...
def main():
    ...
    print("=" * 60)
    print("  Spellcaster Arena — Gesture Magic Engine")
    print("=" * 60)
    print()
    ...
    print()
    print("[READY] Spellcaster Arena is running!")
    print(f"  • Arena URL: http://localhost:3000/spellcaster")
    if phone_mode:
